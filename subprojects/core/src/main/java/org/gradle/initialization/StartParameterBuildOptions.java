...
public class StartParameterBuildOptions {
    ...
    public static class DependencyLockingOption extends BooleanBuildOption<StartParameterInternal> {
        public static final String LONG_OPTION = "write-locks";

        public DependencyLockingOption() {
            super(null, BooleanCommandLineOptionConfiguration.create(LONG_OPTION, "Persists dependency resolution for locked configurations, ignoring existing locking information if it exists", "Disables the persisting of dependency resolution for locked configurations, existing locking information will be used if it exists").incubating());
        }

        @Override
        public void applyTo(boolean value, StartParameterInternal settings, Origin origin) {
            settings.setWriteDependencyLocks(value);
        }
    }
}
