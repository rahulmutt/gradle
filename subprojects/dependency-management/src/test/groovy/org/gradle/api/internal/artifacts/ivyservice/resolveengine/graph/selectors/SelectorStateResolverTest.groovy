...
import static org.gradle.resolve.scenarios.VersionRangeResolveTestScenarios.*
...
class SelectorStateResolverTest extends Specification {
    ...
    def "performs partial resolve when some selectors fail"() {
        ...
        when:
        def selected = selectorStateResolver.selectBest([missingLow, nine, ten, range, missingHigh])
    ...
    def "rethrows failure when all selectors fail to resolve"() {
        ...
        when:
        selectorStateResolver.selectBest([missingLow])

        then:
        thrown(ModuleVersionResolveException)

        when:
        selectorStateResolver.selectBest([missingLow, missingHigh])

        then:
        thrown(ModuleVersionResolveException)

        when:
        selectorStateResolver.selectBest([missingLow, missingHigh, valid])
    ...
    int resolve(VersionRangeResolveTestScenarios.RenderableVersion... versions) {
        ...
        def currentSelection = selectorStateResolver.selectBest(selectors)
